...
package com.aaatech.iap.bb.util;
...
public class PurchaseDatabase {
     ...
     * the "purchased items" table.
     */
    public Cursor getAllPurchasedItems() {
        return mDb.query(PURCHASE_HISTORY_TABLE_NAME, HISTORY_COLUMNS, 
        		HISTORY_STATE_COL + " = " + PurchaseState.PURCHASED.ordinal(),
                null, null, null, null);
    }
     ...
     * 
     */
    public boolean checkSkuIsPurchased() {
    	
    	
    	Cursor cursor = mDb.rawQuery("SELECT * from "+ PURCHASE_HISTORY_TABLE_NAME , null);
    	//Cursor cursor = mDb.rawQuery("SELECT * from "+ PURCHASE_HISTORY_TABLE_NAME +" WHERE "+ HISTORY_PRODUCT_ID_COL +"= '"+ productId + "'", null);
    	 if (cursor == null) {
             return false;
         }
    	 ...
    	 try {
             ...
             while (cursor.moveToNext()) {
            	 ...
            	 int stateIndex = cursor.getInt(cursor.getColumnIndex(HISTORY_STATE_COL));
            	 Log.d(TAG,"Product Index :" + stateIndex);
     ...
     * 
     */
    public boolean checkSkuIsPurchased(String productId) {
